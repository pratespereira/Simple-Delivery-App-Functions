...
describe("Pagina /customer/products", () => {
  ...
  it("Verifica se tela carregou com todos elementos", async () => {
    ...
    const prices = screen.getAllByTestId(
      /customer_products__element-card-price/i
    );
    expect(prices).toHaveLength(11);

    const texts = screen.getAllByTestId(
      /customer_products__element-card-title/i
    );
    expect(texts).toHaveLength(11);

    const plusBtns = screen.getAllByTestId(
      /customer_products__button-card-add-item/i
    );
    expect(plusBtns).toHaveLength(11);

    const minusBtns = screen.getAllByTestId(
      /customer_products__button-card-rm-item/i
    );
    expect(minusBtns).toHaveLength(11);

    const inputs = screen.getAllByTestId(
      /customer_products__input-card-quantity/i
    );
    ...
    expect(
      screen.getByRole("button", { name: /meu carrinho/i })
    ).toBeDisabled();
    expect(
      screen.getByTestId(/customer_products__checkout-bottom-value/i)
