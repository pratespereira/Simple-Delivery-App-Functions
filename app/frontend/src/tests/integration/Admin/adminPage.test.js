...
describe("Testes da página /admin/manage", () => {
  ...
  it("verifica se renderizou a página correta", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("customer_products__element-navbar-user-full-name")
      ).toBeInTheDocument()
    );
    expect(
      screen.getByTestId("customer_products__element-navbar-user-full-name")
    ).toHaveTextContent("Delivery App Admin");
  ...
  it("verifica se insere novo usuário se informados valores corretos", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("admin_manage__input-name")).toBeInTheDocument()
    );
    expect(screen.getAllByRole("row")).toHaveLength(3);

    userEvent.type(
      screen.getByTestId("admin_manage__input-name"),
      NEW_USER.name
    );
    userEvent.type(
      screen.getByTestId("admin_manage__input-email"),
      NEW_USER.email
    );
    userEvent.type(
      screen.getByTestId("admin_manage__input-password"),
      "senhaDifícil"
    );
    userEvent.selectOptions(
      screen.getByTestId("admin_manage__select-role"),
      NEW_USER.role
    );
  ...
  it("verifica se é possível excluir um usuário", async () => {
    ...
    userEvent.click(
      screen.getByTestId("admin_manage__element-user-table-remove-0")
    );
    await waitFor(() => expect(global.fetch).toHaveBeenCalled());
