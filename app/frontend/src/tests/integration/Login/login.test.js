...
describe("Testa a tela de Login", () => {
  ...
  describe("Testa se o botão de LOGIN ativa somente com email e senha no formato válido:", () => {
    beforeEach(() => {
      renderPath("/");
    });
    ...
    it("O botão deve estar desativado com um email no formato incorreto e senha válida.", () => {
      const email = screen.getByRole("textbox", { name: /login/i });
      const password = screen.getByLabelText(/senha/i);
      const login = screen.getByRole("button", { name: /login/i });
    ...
    it(`O botão deve estar desativado com um email no formato válido 
    ...
    it("O botão deve habilitar com um email e senha no formato válido.", () => {
      const email = screen.getByRole("textbox", { name: /login/i });
      const password = screen.getByLabelText(/senha/i);
      const login = screen.getByRole("button", { name: /login/i });
  ...
  it('Verifica se o botão "ainda não tenho conta" redireciona para tela de registro.', () => {
...
describe("Testes de login", () => {
  ...
  it('Logar com usuário { role: "customer" }', async () => {
    ...
    global.fetch.mockResolvedValue({
      json: jest
        .fn()
        .mockResolvedValue(allProductsMock)
        .mockResolvedValueOnce(CUSTOMER_MOCK),
    });
