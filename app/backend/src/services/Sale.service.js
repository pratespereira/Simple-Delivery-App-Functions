  ...
  async create(body) {
    ...
    try {
      const newSaleId = await sequelize.transaction(async (t) => {
        const {
          dataValues: { id },
        } = await this.sale.create({ ...saleData }, { transaction: t });
        const mapped = products.map((p) =>
          this.saleProduct.create({ ...p, saleId: id }, { transaction: t })
        );
        const mappedProducts = products.map(({ productId, quantity }) =>
          this.product.decrement("stockQty", {
            by: quantity,
            where: { id: productId },
            transaction: t,
          })
